...
Ui.BarWidget {
    ...
    function open(payloadJson){if(panelLoader.item)panelLoader.item.open()}
    ...
    Loader { id:panelLoader;active:true;visible:false;source:Qt.resolvedUrl("Panel.qml");onLoaded:{root.inject();Qt.callLater(root.inject)} }
    Ui.WidgetButton {
        id:button
        anchors.fill:parent
        bar:root.bar
        ...
        Ui.OpticalGlyph { anchors.centerIn:parent;anchors.horizontalCenterOffset:root.vertical?0:-Style.spacing.xxs;width:Style.bar.iconCanvas;height:Style.bar.iconCanvas;text:"CH";fontFamily:button.fontFamily;fontSize:button.fontSize;color:button.foreground }
